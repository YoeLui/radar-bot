...
## 3. Diccionario de Archivos (Ecosistema de Datos)
...
## 4. Matriz de Estados Estructurales (`Resultado_Estructura`)
...
## 6. Métricas a Observar en Producción (Fase de Estabilización)
...
* **Uso de la persistencia:** Cantidad de veces que el sistema se ve forzado a recurrir a las marcas temporales `🕒 [Histórico]`.

---

## 7. Planificación del Diseño Futuro (Fase v9.0)
...
## 8. Manual de Contingencias Básicas

### Caso A: El archivo `promos.csv` se corrompió o quedó vacío
El motor cuenta con un validador de integridad. Si la matriz se deforma, el script aborta la sincronización. Para reparar el historial, vaya a la pestaña de Commits en GitHub, descargue la última versión válida de `promos.csv` y vuelva a subirla manualmente al repositorio.
